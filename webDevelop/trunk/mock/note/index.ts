...
Mock.mock(new RegExp('.*' + 'note/updateProject'), () => {
    return {
        status: 1,
    };
});

Mock.mock(new RegExp('.*' + 'note/addProject'), () => {
    return {
        status: 1,
    };
});

Mock.mock(new RegExp('.*' + 'note/deleteProject'), () => {
    return {
        status: 1,
    };
});

Mock.mock(new RegExp('.*' + 'note/getProjectDetail'), () => {
    return {
        status: 1,
        data: {
            projectName: '前端笔记',
...
Mock.mock(new RegExp('.*' + 'note/getNoteInfo'), () => {
    return {
        status: 1,
        data: {
            id: null,

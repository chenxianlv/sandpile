...
import { text } from '@/components/MarkdownParser/temp';
...
const inputHandler = debounce(function () {
    ...
    markdownText.value = textareaRef.value.value;
}, 200);
...
const updateMenu = (menus: string[]) => {
    markdownMenus.value = menus;
};
...
<template>
    ...
    <div class="content">
        ...
        ></textarea>
        <div>
            <MarkdownParser :markdownText="markdownText" :updateMenu="updateMenu" />
        </div>
    </div>
</template>

<!--<style src="./Markdown.less" scoped></style>-->
